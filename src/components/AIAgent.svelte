<script>
    ...
    import botIcon from "$lib/assets/chat-svgrepo-com.svg";
    ...
    const getAllChat = () => {
        try{
            return localStorage.getItem("allAIAentChats") ? JSON.parse(localStorage.getItem("allAIAentChats")) : "No chat available";
        }catch(err){
            console.log(err)
        }
    }
    ...
    const storeChat = (prompt, reply) => {
        ...
        const allChats = localStorage.getItem("allAIAentChats") ? JSON.parse(localStorage.getItem("allAIAentChats")) : localStorage.setItem("allAIAentChats",JSON.stringify([chat]));

        localStorage.setItem("allAIAentChats", JSON.stringify([...allChats, chat]));
...
<section class="{openChat ? 'translate-y-0' : 'translate-y-500'} tansition-transform duration-1000 transform flex fixed flex-col justify-start z-40 bottom-0 right-0 w-full h-full sm:w-[400px] sm:h-[400px] sm:drop-shadow-xl sm:rounded-lg sm:overflow-hidden sm:bottom-5 sm:right-5 bg-white">
    <div class="py-2 w-full bg-[#2a6be1] text-white text-2xl text-center flex items-center justify-between px-5 space-y-2">
        <div class="flex flex-col space-y-2 items-center justify-center">
            <h2><span class="uppercase text-[#fff]">SVA</span> Chatbot</h2>
...
<section class="{openChat ? 'opacity-0 hide' : 'show opacity-100'} transition-opacity duration-1000 fixed bottom-5 right-5 sm:bottom-10 sm:bottom-10 z-40 shadow-lg rounded-full bg-white">
    <div onclick={() => hundleOpenChat()} role="button" tabindex="0" onkeydown={() => openChat} class="w-[70px] sm:w-[80px] cursor-pointer">
        <img src={botIcon} alt="SVA Agent" class="w-full h-full object-cover" />
    </div>
</section>
